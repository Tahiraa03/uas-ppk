...
<div class="container">
	...
	<table id="tabeldata" width="100%" class="table table-striped table-bordered">
		...
		<tbody>
			<?php
			$no = 1;
			$storeNames = array(); // Array untuk menyimpan nama alternatif

			// Mengambil data nilai V dan menyimpan nama alternatif
			while ($row = mysqli_fetch_array($query)) {
				$idAlternatif = $row['id_alternatif'];
				$nilaiV = $row['nilaiV'];
				$storeNames[$idAlternatif] = $row['nm_alternatif']; // Menyimpan nama alternatif

				// Menampilkan data pada tabel (tanpa perubahan)
				echo "<tr>
                        <td>{$no}</td>
                        <td>{$idAlternatif}</td>
                        <td>{$storeNames[$idAlternatif]}</td>
                        <td>{$nilaiV}</td>
                        <td>{$no}</td>
                        <td class='text-center'>
                            <a href='edit-kriteria.php?id={$idAlternatif}' class='btn btn-warning text-white'><i class='fas fa-pencil-alt'></i></a>
                            <a href='hapus-kriteria.php?id={$idAlternatif}' onclick=\"return confirm('Yakin ingin menghapus data')\" class='btn btn-danger'><span class='fas fa-trash-alt'></span></a>
                        </td>
                    </tr>";
				$no++;
			}
